...
final class DraftService
{
    public function create(int $itemId): int
    {
        ...
        try{
            ...
            $content=ContentSanitizer::clean((string)$output['content_html']).'<p><strong>'.esc_html__('Source:','wordpress-news-bot').'</strong> '.esc_html((string)$item['source_name']).' - <a rel="nofollow noopener" target="_blank" href="'.esc_url((string)$item['source_url']).'">'.esc_html__('Source link','wordpress-news-bot').'</a></p>';
            ...
            update_post_meta($postId,'_wpnb_feed_item_id',$itemId);update_post_meta($postId,'_wpnb_source_url',esc_url_raw((string)$item['source_url']));update_post_meta($postId,'_wpnb_source_name',sanitize_text_field((string)$item['source_name']));update_post_meta($postId,'_wpnb_content_hash',sanitize_text_field((string)$item['content_hash']));update_post_meta($postId,'_wpnb_ai_provider',$generation['provider']);update_post_meta($postId,'_wpnb_ai_model',$provider->model());update_post_meta($postId,'_wpnb_generated_at',Support::now());wp_set_post_tags($postId,$output['suggested_tags']);
