...
final class DraftService
{
    public function create(int $itemId): int
    {
        ...
        $lock = 'wpnb_draft_lock_' . $itemId; if (get_transient($lock)) throw new \RuntimeException('Bu haber başka bir işlem tarafından işleniyor.'); set_transient($lock, 1, 5 * MINUTE_IN_SECONDS);
        global $wpdb; try {
            ...
            $existing = get_posts(['post_type'=>'post','post_status'=>['draft','pending','publish','future'],'numberposts'=>1,'fields'=>'ids','meta_query'=>['relation'=>'OR',['key'=>'_wpnb_feed_item_id','value'=>(string)$itemId],['key'=>'_wpnb_source_url','value'=>esc_url_raw($item['source_url'])],['key'=>'_wpnb_content_hash','value'=>sanitize_text_field($item['content_hash'])]]]); if ($existing) { $wpdb->update(Support::table('feed_items'), ['status'=>'duplicate','updated_at'=>Support::now()], ['id'=>$itemId], ['%s','%s'], ['%d']); return (int) $existing[0]; }
            $settings = get_option('wpnb_settings', []); if ((int) ($settings['daily_ai_quota'] ?? 100) <= $this->todayUsage()) throw new \RuntimeException('Günlük AI kotası doldu.');
            ...
            update_post_meta($postId, '_wpnb_feed_item_id', $itemId); update_post_meta($postId, '_wpnb_source_url', esc_url_raw($item['source_url'])); update_post_meta($postId, '_wpnb_source_name', sanitize_text_field($item['source_name'])); update_post_meta($postId, '_wpnb_content_hash', sanitize_text_field($item['content_hash'])); update_post_meta($postId, '_wpnb_ai_provider', $provider instanceof OpenAiProvider ? 'openai' : 'mock'); update_post_meta($postId, '_wpnb_ai_model', $provider->model()); update_post_meta($postId, '_wpnb_generated_at', Support::now()); wp_set_post_tags($postId, $output['suggested_tags']);
