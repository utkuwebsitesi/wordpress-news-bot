    ...
    public function inspect(): array
    {
        ...
        foreach(DatabaseSchema::tables()as$logical=>$spec){$table=Support::table($logical);$exists=$this->tableExists($table);$tableInfo=['exists'=>$exists,'columns'=>[],'indexes'=>[],'engine'=>'','collation'=>''];
            ...
            try{
                ...
                $indexes=(array)$this->db->get_results('SHOW INDEX FROM '.DatabaseSchema::identifier($table),ARRAY_A);$grouped=[];foreach($indexes as$row){$key=(string)($row['Key_name']??'');$seq=(int)($row['Seq_in_index']??1);$grouped[$key]['unique']=((int)($row['Non_unique']??1)===0);$grouped[$key]['columns'][$seq]=(string)($row['Column_name']??'');}foreach($grouped as$key=>$index){ksort($index['columns']);$tableInfo['indexes'][$key]=['unique'=>$index['unique'],'columns'=>array_values($index['columns'])];}
                foreach($spec['indexes']as$name=>$expected){$actual=$tableInfo['indexes'][$name]??null;if(!$actual){$issues[]=['code'=>'index_missing','table'=>$logical,'index'=>$name];}elseif($actual['unique']!==$expected['unique']||$actual['columns']!==$expected['columns']){$issues[]=['code'=>'index_mismatch','table'=>$logical,'index'=>$name];}}
                $status=(array)$this->db->get_row($this->db->prepare('SHOW TABLE STATUS LIKE %s',$table),ARRAY_A);$tableInfo['engine']=(string)($status['Engine']??'');$tableInfo['collation']=(string)($status['Collation']??'');if($tableInfo['engine']!==''&&strtolower($tableInfo['engine'])!=='innodb')$issues[]=['code'=>'engine_mismatch','table'=>$logical,'actual'=>$tableInfo['engine']];
                $expectedCharset=strtolower((string)($this->db->charset??''));if($expectedCharset!==''&&$tableInfo['collation']!==''&&!str_starts_with(strtolower($tableInfo['collation']),$expectedCharset.'_'))$issues[]=['code'=>'collation_mismatch','table'=>$logical,'actual'=>$tableInfo['collation']];
        ...
        $fingerprint=substr(hash('sha256',Support::json($safeTables)),0,24);
