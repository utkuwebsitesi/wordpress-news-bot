...
final class CronHealth
{
    ...
    public static function isHealthy(int$maxAge=5*MINUTE_IN_SECONDS):bool
    {
        $last=self::lastSuccess();$timestamp=$last===''?false:strtotime($last);
        return$timestamp!==false&&max(0,time()-$timestamp)<=$maxAge;
    }
    ...
    public static function testConnection():array
    {
        ...
        $url=(string)apply_filters('wpnb_cron_test_url',site_url('/wp-cron.php'));
        ...
        DiagnosticStore::record($testId,'cron','success','heartbeat_test',['http_status'=>$status]);
