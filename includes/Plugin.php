...
final class Plugin
{
    ...
    public function boot(): void { load_plugin_textdomain('wordpress-news-bot', false, dirname(plugin_basename(WPNB_FILE)).'/languages'); if ((string)get_option('wpnb_schema_version','')!==WPNB_SCHEMA_VERSION||(string)get_option('wpnb_plugin_version','')!==WPNB_VERSION) Database::activate(); (new Admin())->register(); add_action('wpnb_poll_sources', [$this, 'poll']); add_action('wpnb_sources_polled', [$this, 'importActiveSources']); add_action('update_option_wpnb_settings', [$this, 'syncCron'], 10, 3); }
