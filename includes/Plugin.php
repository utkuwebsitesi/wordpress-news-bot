...
final class Plugin
{
    ...
    public function boot(): void { load_plugin_textdomain('wordpress-news-bot', false, dirname(plugin_basename(WPNB_FILE)).'/languages'); if ((string) get_option('wpnb_schema_version', '') !== WPNB_SCHEMA_VERSION && !get_option('wpnb_source_recovery_required')) Database::activate(); (new Admin())->register(); add_action('wpnb_poll_sources', [$this, 'poll']); add_action('wpnb_sources_polled', [$this, 'importActiveSources']); add_action('update_option_wpnb_settings', [$this, 'syncCron'], 10, 3); }
    ...
    public function importActiveSources(): void { global $wpdb; $ids=(array)$wpdb->get_col('SELECT id FROM '.Support::table('sources').' WHERE active=1 ORDER BY id ASC'); foreach($ids as$id){try{(new SourceImporter())->import((int)$id);}catch(\Throwable$e){$wpdb->update(Support::table('sources'),['last_checked_at'=>Support::now(),'last_result'=>__('Failed','wordpress-news-bot'),'last_error'=>__('The source could not be imported. Check the RSS URL and try again.','wordpress-news-bot'),'updated_at'=>Support::now()],['id'=>(int)$id]);$wpdb->insert(Support::table('logs'),['level'=>'error','event'=>'source_import_failed','message'=>'A source import failed.','context_json'=>Support::json(Security::cleanLogContext(['source_id'=>(int)$id,'exception_class'=>get_class($e)])),'created_at'=>Support::now()]);}} }
}
