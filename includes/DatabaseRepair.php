...
final class DatabaseRepair
{
    ...
    private function replaceMismatchedIndex(string$logical,string$name,bool$automatic):bool
    {
        if($name==='PRIMARY')return false;$index=DatabaseSchema::tables()[$logical]['indexes'][$name]??null;if(!$index)return false;$table=Support::table($logical);if($index['unique']){$cols=implode(',',array_map([DatabaseSchema::class,'identifier'],$index['columns']));$duplicates=(int)$this->db->get_var('SELECT COUNT(*) FROM (SELECT 1 FROM '.DatabaseSchema::identifier($table).' GROUP BY '.$cols.' HAVING COUNT(*)>1 LIMIT 1) wpnb_duplicates');if($duplicates>0)return false;if($automatic&&$logical==='sources'&&$name==='canonical_hash_unique'&&(int)$this->db->get_var('SELECT COUNT(*) FROM '.DatabaseSchema::identifier($table)." WHERE canonical_hash='' ")>0)return false;}$cols=implode(',',array_map([DatabaseSchema::class,'identifier'],$index['columns']));$sql='ALTER TABLE '.DatabaseSchema::identifier($table).' DROP INDEX '.DatabaseSchema::identifier($name).', ADD '.($index['unique']?'UNIQUE ':'').'INDEX '.DatabaseSchema::identifier($name).' ('.$cols.')';$this->mustQuery($sql);return true;
    }
