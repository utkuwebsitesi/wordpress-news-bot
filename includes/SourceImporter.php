...
final class SourceImporter
{
    public function import(int $sourceId): int
    {
        ...
        $items=(new FeedParser())->parse(wp_remote_retrieve_body($response)); $count=0; $detector=new DuplicateDetector($wpdb); foreach($items as $item){ if($detector->isDuplicate($item,$sourceId)) continue; $now=Support::now(); $wpdb->insert(Support::table('feed_items'),['source_id'=>$sourceId,'source_name'=>(string)$source['name'],'source_feed_url'=>(string)$source['feed_url'],'guid'=>sanitize_text_field($item['guid']),'source_url'=>esc_url_raw($item['source_url']),'normalized_url'=>Support::normalizeUrl($item['source_url']),'title'=>sanitize_text_field($item['title']),'excerpt'=>sanitize_textarea_field($item['excerpt']),'content_hash'=>$item['content_hash'],'status'=>'new','raw_data'=>null,'created_at'=>$now,'updated_at'=>$now]); $count++; }
        $wpdb->update(Support::table('sources'),['last_success'=>Support::now(),'last_checked_at'=>Support::now(),'last_result'=>sprintf(__('Imported: %d','wordpress-news-bot'),$count),'last_error'=>null,'updated_at'=>Support::now()],['id'=>$sourceId]); return $count;
    }
}
