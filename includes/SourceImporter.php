...
final class SourceImporter
{
    ...
    private function importLocked(int $sourceId,int$limit):array
    {
        ...
        foreach(array_slice($result['items'],0,$limit)as$item){
            ...
            $now=Support::now();
            $imageUrl=esc_url_raw((string)($item['image_url']??''));$record=['source_id'=>$sourceId,'source_name'=>(string)$source['name'],'source_feed_url'=>(string)$source['feed_url'],'guid'=>$guid,'source_url'=>$url,'normalized_url'=>Support::normalizeUrl($url),'title'=>$title,'excerpt'=>sanitize_textarea_field((string)($item['excerpt']??'')),'source_category'=>sanitize_text_field((string)($item['source_category']??'')),'wordpress_category_id'=>(int)($source['category_id']??0),'content_hash'=>$hash,'image_url'=>$imageUrl,'image_source'=>sanitize_key((string)($item['image_source']??'')),'image_status'=>'missing','image_attachment_id'=>0,'image_hash'=>'','image_error_code'=>'','published_at'=>$this->mysqlDate((string)($item['published_at']??'')),'status'=>'new','raw_data'=>null,'created_at'=>$now,'updated_at'=>$now];
