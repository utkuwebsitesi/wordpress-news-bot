...
final class FeedParser
{
    public function parse(string $xml): array
    {
        ...
        foreach ($nodes as $item) {
            ...
            $items[] = ['guid' => $guid, 'source_url' => esc_url_raw($link), 'title' => $title, 'excerpt' => wp_trim_words(wp_strip_all_tags($description), 55), 'content_hash' => hash('sha256', preg_replace('/\s+/u', ' ', wp_strip_all_tags($description)) ?: $title), 'published_at'=>$published];
        }
        return $items;
    }
}
