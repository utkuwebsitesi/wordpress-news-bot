...
final class MariaDbWpdb
{
    ...
    public function get_results(string$sql,mixed$format=null):array{try{return$this->pdo->query($sql)->fetchAll();}catch(\PDOException$e){$this->capture($e);return[];}}
    ...
    public function query(string$sql):int|false{try{$this->clear();return$this->pdo->exec($sql);}catch(\PDOException$e){$this->capture($e);return false;}}
    ...
    public function reset():void{foreach(array_reverse(array_keys(\WordPressNewsBot\DatabaseSchema::tables()))as$logical)$this->pdo->exec('DROP TABLE IF EXISTS `'.$this->prefix.'wpnb_'.$logical.'`');$this->clear();}
