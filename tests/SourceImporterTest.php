...
final class SourceImporterTest extends TestCase
{
    ...
    {
        $xml='<rss xmlns:media="http://search.yahoo.com/mrss/"><channel><item><guid>1</guid><title>One</title><link>https://feed.example/items/1</link><media:content url="https://feed.example/image.jpg" type="image/jpeg"/></item><item><guid>2</guid><title>Two</title><link>https://feed.example/items/2</link></item></channel></rss>';
        ...
        $summary=(new SourceImporter($tester,$db))->importDetailed(7);$this->assertSame(['read'=>2,'new'=>2,'duplicate'=>0,'invalid'=>0,'failed'=>0],array_intersect_key($summary,array_flip(['read','new','duplicate','invalid','failed'])));$this->assertCount(2,$db->feedInserts);$this->assertCount(23,$db->feedFormats);$this->assertSame(0,$db->feedInserts[0]['wordpress_category_id']);$this->assertSame(0,$db->feedInserts[0]['wordpress_post_id']);$this->assertSame('One',$db->feedInserts[0]['title']);$this->assertSame('media:content',$db->feedInserts[0]['image_source']);$this->assertSame('2 read, 2 new, 0 duplicate.',$db->sourceUpdate['last_result']);
    }
...
final class ImportWpdb
{
    ...
    public function get_row(string$sql,mixed$output=null):array{return['id'=>7,'active'=>1,'name'=>'Fixture','feed_url'=>'https://feed.example/rss','allowed_domains'=>'feed.example','daily_quota'=>$this->quota,'import_images'=>0];}
