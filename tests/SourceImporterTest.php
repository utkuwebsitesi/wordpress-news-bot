...
final class SourceImporterTest extends TestCase
{
    ...
    public function testSecureFetcherQuotaAndCanonicalFormatsAreUsed():void
    {
        ...
        $this->assertSame(1,(new SourceImporter($tester,$db))->import(7));$this->assertCount(1,$db->feedInserts);$this->assertSame(['%d','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s'],$db->feedFormats);$this->assertSame('One',$db->feedInserts[0]['title']);$this->assertSame('Imported 1 new items.',$db->sourceUpdate['last_result']);
    }
