...
final class SourceImporterTest extends TestCase
{
    ...
    public function testSecureFetcherDetailedSummaryCategoryAndCanonicalFormatsAreUsed():void
    {
        ...
        $summary=(new SourceImporter($tester,$db))->importDetailed(7);$this->assertSame(['read'=>2,'new'=>2,'duplicate'=>0,'invalid'=>0,'failed'=>0],array_intersect_key($summary,array_flip(['read','new','duplicate','invalid','failed'])));$this->assertCount(2,$db->feedInserts);$this->assertCount(22,$db->feedFormats);$this->assertSame(0,$db->feedInserts[0]['wordpress_category_id']);$this->assertSame('One',$db->feedInserts[0]['title']);$this->assertSame('media:content',$db->feedInserts[0]['image_source']);$this->assertSame('2 read, 2 new, 0 duplicate.',$db->sourceUpdate['last_result']);
    }
