...
final class SourceConnectionTesterTest extends TestCase
{
    ...
    public function testNtvLikeApplicationXmlAtomFixture():void{$result=$this->tester($this->response(200,self::ATOM,'application/xml'))->test('https://www.example.com/turkiye.rss',['example.com']);$this->assertSame('Atom',$result['feed_type']);$this->assertSame(1,$result['item_count']);$this->assertSame('2025-01-01T10:00:00Z',$result['last_item_date']);$this->assertSame('www.example.com',$result['final_host']);$this->assertNotEmpty($result['test_id']);}
